...
-- Automatically captures form submissions and creates user records
-- All metrics derived from analytics_events and users tables
...
DROP VIEW IF EXISTS top_interactions_view CASCADE;
...
DROP TRIGGER IF EXISTS trigger_create_user_from_form ON analytics_events;
DROP FUNCTION IF EXISTS update_session_data() CASCADE;
DROP FUNCTION IF EXISTS create_user_from_form() CASCADE;
...
-- Create function to automatically create user from form submission
CREATE OR REPLACE FUNCTION create_user_from_form()
RETURNS TRIGGER AS $$
BEGIN
  -- Only create user if this is a form submission event
  IF NEW.event_type IN ('form_completed', 'signup_completed', 'form_submit') THEN
    -- Extract form data
    INSERT INTO users (name, city, email, selected_plan, signup_source)
    VALUES (
      COALESCE(NEW.event_data->>'name', 'Unknown'),
      COALESCE(NEW.event_data->>'city', 'Unknown'),
      COALESCE(NEW.event_data->>'email', NEW.event_data->>'user_email'),
      COALESCE(NEW.event_data->>'plan', NEW.event_data->>'selected_plan', 'free'),
      COALESCE(NEW.event_data->>'source', NEW.event_data->>'signup_source', 'form')
    )
...
-- Create trigger to automatically create user from form submission
CREATE TRIGGER trigger_create_user_from_form
  AFTER INSERT ON analytics_events
  FOR EACH ROW
  EXECUTE FUNCTION create_user_from_form();
